...
constexpr std::array<uint8_t, 8> XARY_BINARY_MAGIC = {0x8F, 0x1E, 0xAA, 0x4D,
                                                      0x9C, 0x05, 0xF3, 0x72};
...
// Proprietary Encoding Cipher (Only Xary's mathematical inverse can reverse
// this)
void encodeBuffer(std::vector<uint8_t> &buffer,
                  std::size_t globalStreamOffset) {
  for (std::size_t i = 0; i < buffer.size(); ++i) {
    std::size_t pos = globalStreamOffset + i;
    uint8_t key = XARY_MASTER_KEY[pos % XARY_MASTER_KEY.size()] ^
...
void decodeBuffer(std::vector<uint8_t> &buffer,
                  std::size_t globalStreamOffset) {
  for (std::size_t i = 0; i < buffer.size(); ++i) {
    std::size_t pos = globalStreamOffset + i;
    uint8_t key = XARY_MASTER_KEY[pos % XARY_MASTER_KEY.size()] ^
...
int main(int argc, char *argv[]) {
  ...
  case cli::Mode::Inspect: {
    std::cout << "[🔍] Inspecting binary signature: " << options.inputFile
              << "\n\n";
    ...
    // Wrap header in zero-copy BufferView
    ...
    std::string currentFile = options.inputFile;
    if (info.isKnown &&
        currentFile.find(info.expectedExtension) == std::string::npos) {
      std::cout
          << "\n⚠️  WARNING: File extension appears corrupt or mismatched!\n";
      std::cout << "👉 Suggested Fix : Rename file to end with '"
                << info.expectedExtension << "'\n";
  ...
  case cli::Mode::Encode: {
    std::string outPath = options.outputFile.empty()
                              ? (options.inputFile + ".xary")
                              : options.outputFile;
    ...
    std::cout << "✔ Encoded " << totalBytes << " bytes across " << chunks
              << " chunk(s).\n";
    return 0;
  }
  ...
  case cli::Mode::Decode: {
    std::string outPath = options.outputFile.empty()
                              ? (options.inputFile + ".out")
                              : options.outputFile;
    std::cout << "[+] Processing/Decoding: " << options.inputFile << " -> "
              << outPath << "\n";
    ...
    if (isXaryFile) {
      ...
      if (firstChunk.size() > XARY_BINARY_MAGIC.size()) {
        std::vector<uint8_t> payload(
            firstChunk.begin() + XARY_BINARY_MAGIC.size(), firstChunk.end());
      ...
      std::cout << "✔ Decoded " << totalBytes << " bytes across " << chunks
                << " chunk(s).\n";
    } else {
      std::cout << "ℹ Raw binary stream detected (Non-Xary format). Streaming "
                   "raw pass-through...\n";
      ...
      std::cout << "✔ Streamed " << totalBytes
                << " raw binary bytes successfully.\n";
    }
